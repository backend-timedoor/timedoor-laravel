...
- Format changed PHP files with Laravel Pint, 4-space indentation (Pint's `laravel` preset default). Run Pint only against files changed in the current task; do not reformat unrelated files — see [[workflow]].
